...
import Formularios.JDialogVer_Venta;
...
public class frmCaja extends javax.swing.JFrame {
    ...
    public frmCaja() {
        ...
        initComponents();
        setIconImage(new ImageIcon(getClass().getResource("/imagenes/caja.png")).getImage());
        nombresTitulos();
    }
    ...
    public void indivdual(){
        int filas;
        int n = tblvntasC.getSelectedRow();
        ...
        else if(control.VerificaConsulta("SELECT v.idventa_confec FROM venta_confec v INNER JOIN comprobante c on v.fk_idComprobante=c.idComprobante WHERE c.fk_Num_factura='"+fact+"'")){
            String fac=control.DevolverRegistroDato("SELECT v.fk_idComprobante FROM venta_confec v INNER JOIN comprobante c on v.fk_idComprobante=c.idComprobante WHERE c.fk_Num_factura='"+fact+"'", 1);
            int num=Integer.parseInt(fac);
            try {
                Conexion conn=new Conexion();
                Connection con=conn.conectar();
        ...
        else if(control.VerificaConsulta("SELECT v.idventa_confec FROM venta_confec v INNER JOIN comprobante c on v.fk_idComprobante=c.idComprobante WHERE c.fk_Num_boleta='"+fact+"'")){
            String bole=control.DevolverRegistroDato("SELECT v.fk_idComprobante FROM venta_confec v INNER JOIN comprobante c on v.fk_idComprobante=c.idComprobante WHERE c.fk_Num_boleta='"+fact+"'", 1);
            int num=Integer.parseInt(bole);
            try {
                Conexion conn=new Conexion();
                Connection con=conn.conectar();
    ...
    public void reported(){
        try {
            ...
            Connection con=conn.conectar();
            JasperReport reporte=null;
            String path="src\\Reportes\\rTotal.jasper";
            Map parametro=new HashMap();
            
            parametro.put("dia", jDateChooserFecha.getDate());
    ...
    private void initComponents() {
        ...
        getContentPane().add(pnel_cab2, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 30, 890, 440));
